...
//  ChatNasrCity
//
//  Created by Ahmed Shoman on 12/4/19.
...
func createChatRoomId(chatRomIds: [String]) -> String{
    
    let compare = chatRomIds.first!.compare(chatRomIds.last!).rawValue
    
    if compare < 1{
        return chatRomIds.first! + chatRomIds.last!
    }else{
        return chatRomIds.last! + chatRomIds.first!
    }
}
